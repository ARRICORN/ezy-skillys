...
export async function POST(req) {
  try {
    ...
    if (!password?.length || password.length < 5) {
      return Response.json({ message: "Password must be at least 5 characters" });
    }
    ...
    if (isUserRegistered) {
      return  Response.json({ message: "You are already registered as a regular user. Cannot register as a User." });
    }
    ...
    return Response.json(
      { User: createdUser[0], UserInfo: createdUserInfo[0] },
      {
        headers: {
          'Set-Cookie': cookie,
        },
